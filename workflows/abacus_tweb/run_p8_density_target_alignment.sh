#!/usr/bin/env bash
# Allocation-owned CPU preflight for the P8.9 observer-frame density target.
...
OUT=/pscratch/sd/d/dkololgi/abacus/p8_density_phys_v1/preflight/coordinate_alignment.json
LOG=/pscratch/sd/d/dkololgi/logs/p8_density_target_alignment_supervisor.log
...
salloc \
  ...
  "$PY" -u -m workflows.abacus_tweb.p8_density_target_alignment \
  >> "$LOG" 2>&1
STATUS=$?
set -e
