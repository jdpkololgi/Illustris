...
# Columns present on SecondGen cosmosim forFA*.fits (from upstream_prepare_mocks_Y3_bright); joined
# onto FASSIGN/FAVAIL combinatorics and read from pota-*.fits when usepota=y.
FORFA_BRIGHT_TAGALONG = (
    'TRUEZ',
    'RSDZ',
    'Z_COSMO',
    'Z',
...
tile_fn = os.getenv(
    'LSS_TILES_FITS',
    '/global/cfs/cdirs/desi/survey/catalogs/' + survey + '/LSS/tiles-' + pr + '.fits',
)
...
if args.mockver == 'ab_secondgen' and args.combd == 'y':
    ...
    def _get_fa(tile):
        ...
        fa['TILEID'] = int(tile)
        return fa
    # --usepota y: skip FASSIGN (no Univ000/fa/MAIN fiberassign); joindspec loads assignwdup if needed
    if args.usepota == 'n':
        pa_hdu = 'FASSIGN'
        addcols = ['TARGETID', 'RSDZ', 'ZWARN'] + [c for c in FORFA_BRIGHT_TAGALONG if c not in ('RSDZ', 'ZWARN')]
        tl = []    
        tls = tiles['TILEID']
        if args.par == 'n':
    ...
    if args.usepota == 'n':
        ...
        if pdir == 'bright':
            ...
            for c in FORFA_BRIGHT_TAGALONG:
                if c not in addcols:
                    addcols.append(c)
...
if args.joindspec == 'y':
    ...
    # pota and assignwdup share tagalongs; drop duplicates from pa so join keeps asn values
    _join_keys = {'TARGETID', 'LOCATION', 'TILEID'}
    for c in list(pa.colnames):
        if c in asn.colnames and c not in _join_keys:
            pa.remove_column(c)
    tj = join(pa, asn, keys=['TARGETID', 'LOCATION', 'TILEID'], join_type='left')
    z = np.ma.asarray(tj['ZWARN'], dtype=np.int64)
    tj['ZWARN'] = z.filled(999999)
    sel = tj['ZWARN'] == 999999
...
nzmd = 'mock'
mainp = main(args.tracer, args.specdata, survey=args.survey, relax_zbounds='y')
...
if args.mkclusdat == 'y':
    ...
    mkclus_extracols = [c for c in FORFA_BRIGHT_TAGALONG if c != 'ZWARN']
    if args.tracer[:3] == 'BGS':
        ct.mkclusdat(
            os.path.join(readdir, args.tracer + notqso),
            weightileloc,
            tp=args.tracer,
            dchi2=None,
            zmin=mainp.zmin,
            zmax=mainp.zmax,
            use_map_veto=args.use_map_veto,
            subfrac=subfrac,
            zsplit=zsplit,
            ismock=True,
            ccut=args.ccut,
            logger=logger,
            exttp='.fits',  # cosmic_env lacks hdf5plugin; FITS is sufficient for mocks
            extracols=mkclus_extracols,
        )
    else:
        ct.mkclusdat(
            os.path.join(readdir, args.tracer + notqso),
    ...
    #This is for bright Uchuu:  ct.mkclusdat(os.path.join(readdir, args.tracer + notqso), weightileloc, tp=args.tracer, dchi2= None, zmin=mainp.zmin, zmax=mainp.zmax, use_map_veto=args.use_map_veto, subfrac=subfrac, zsplit=zsplit, ismock=True, ccut=args.ccut,logger=logger,exttp='.h5', extracols=['ABSMAG_R', 'REST_GMR_0P1']) #, return_cat='y', write_cat='n')
