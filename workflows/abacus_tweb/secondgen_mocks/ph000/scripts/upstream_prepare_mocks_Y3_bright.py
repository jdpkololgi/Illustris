...
# CutSky BGS extension 1 — verified with fitsio on
# /global/cfs/cdirs/desi/cosmosim/SecondGenMocks/AbacusSummit/CutSky/BGS/v0.1/z0.200/
# cutsky_BGS_z0.200_AbacusSummit_base_c000_ph000.fits (2026-05-13).
# Exact names and numpy dtypes from fitsio.read(..., rows=0):
#   R_MAG_APP, R_MAG_ABS, G_R_REST, G_R_OBS, HALO_MASS, Z_COSMO, Z  -> >f4
#   RA, DEC  -> >f8
#   CEN, RES, FILE_NUM, HALO_INDEX, BOX_INDEX,
#   IN_Y1, NGC_Y1, SGC_Y1, N_Y1, S_Y1, IN_Y5, NGC_Y5, SGC_Y5, N_Y5, S_Y5  -> >i4 (int32)
# If a user checklist assumed int16 for bitmask/linkage columns, note the on-disk int32.
...
parser.add_argument("--isProduction", help="Say yes if you want to save in main production directory",default='y')
...
tiletab = Table.read('/global/cfs/cdirs/desi/survey/catalogs/DA2/LSS/tiles-{PROG}.fits'.format(PROG = args.prog.upper()))
...
if args.isProduction == 'y':
    args.base_output = '/global/cfs/cdirs/desi/survey/catalogs/DA2/mocks'
    args.overwrite = False
else:
    if args.base_output == '/global/cfs/cdirs/desi/survey/catalogs/DA2/mocks' or args.base_output == '/global/cfs/cdirs/desi/survey/catalogs/DA2/mocks/':
...
#            out_file_name = os.path.join(mockdir, 'forFA{real}.fits'.format(real=real))
    ...
    for type_ in types:
        if args.mockver == 'ab_firstgen' or args.mockver == 'ab_secondgen':
            ...
            print('thepath')
            print(thepath)
            data = Table(fitsio.read(thepath, columns=list(CUTSKY_BGS_SECONDGEN_COLS)))
    ...
    if args.apply_mask == 'y':
        ...
        maskv = mask.get_nobsandmask(
            nproc=int(os.getenv('IMAGING_NOBS_NPROC', '32')))
    ...
    n=len(targets)
    # Canonical CutSky names: Z_COSMO (true), Z (observed/RSD). TRUEZ/RSDZ duplicate those for LSS/fiberassign.
    targets['TRUEZ'] = np.asarray(targets['Z_COSMO'], dtype=np.float32)
    targets['RSDZ'] = np.asarray(targets['Z'], dtype=np.float32)
    # mkCat_SecondGen_amtl bright path reads ABSMAG_R / REST_GMR_0P1 from pota joins — alias if missing.
    if 'R_MAG_ABS' in targets.colnames and 'ABSMAG_R' not in targets.colnames:
        targets['ABSMAG_R'] = np.asarray(targets['R_MAG_ABS'], dtype=np.float32)
    if 'G_R_REST' in targets.colnames and 'REST_GMR_0P1' not in targets.colnames:
        targets['REST_GMR_0P1'] = np.asarray(targets['G_R_REST'], dtype=np.float32)
    if args.prog == 'dark':
...
# apply_mask uses an inner Pool in LSS imaging; cannot nest under this Pool (daemon workers).
if args.realmax - args.realmin <= 1:
