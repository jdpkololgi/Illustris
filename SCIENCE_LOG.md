...
### 2026-08-03 — [code] Docs: P8 recovery / extension resume / P9 operational notes
...
warm-start contract, mid-epoch `--resume` safeguards, recovery plotters, and P9
residual-blend diagnostic are discoverable from the canonical index/runbook
(prior draft PRs #12/#13 never merged; #12 still claimed P8.5 unimplemented).
Distinguishes short-screen vs `recovery_v1` vs extension run classes; records
linear-increment vs softplus target split, resume Git/CLI freeze, warm-start
sibling provenance, and DESI `PYTHONPATH` pitfall.

- What: documentation-only sync for P8/P9 entrypoints and constraints
- Why / decision: recovery + extension + resume landed after the last merged
  docs automation; operators still had no runbook beyond `SCIENCE_LOG.md` /
  plan checklists
- Next: refresh runbook status when extension audits close or P10 opens;
  still-open docs draft PR #11 covers S0 / F-tier diagnostics
