...
- Generalisable-GraphWeb P8 spatial-transfer screen and recovery:
  ...
  - Current status: two-rotation `recovery_v1` complete (U-PATCH clears both
    registered bars; G-PATCH fails classical supported-shell bar). Primary
    artifacts are immutable; `convergence_extension_v1` is the in-flight
    long-horizon optimization diagnostic. Same-phase evidence only — P10
    remains the production-transfer gate. See `RUNBOOK.md` and
    `docs/plan_generalisable_graphweb_vac.md` §P8/P9.
...
- P8 recovery `--resume` freezes the checkpoint Git revision: resume from a
  detached worktree at that SHA rather than weakening the guard. Clear inherited
  DESI `PYTHONPATH`/`PYTHONHOME` before launching interactive recovery jobs.
