...
def successful_rows(data,*,z_column='Z_not4clus',mock=False):
    """Pinned legacy GraphWeb quality rule; mock catalogues have no noisy spectra."""
    z=np.asarray(data[z_column]);good=np.isfinite(z)&(z>0)&(data['ZWARN']==0)
    if not mock:
        good &= (data['DELTACHI2']>=25)&(np.char.strip(data['SPECTYPE'].astype('U'))=='GALAXY')
    return good
