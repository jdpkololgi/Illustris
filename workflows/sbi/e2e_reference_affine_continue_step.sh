...
srun --jobid="$job_id" --nodes=1 --ntasks=4 --cpus-per-task=16 \
    ...
    "$python_bin" -u -m workflows.sbi.e2e_reference_affine_continue worker --output "$run_root"
srun --jobid="$job_id" --nodes=1 --ntasks=1 --cpus-per-task=16 --gpus=1 \
    "$python_bin" -u -m workflows.sbi.e2e_reference_affine_continue collect --output "$run_root"
