...
srun --jobid="$job_id" -N1 -n4 -c16 --gpus-per-task=1 --gpu-bind=single:1 --cpu-bind=cores --kill-on-bad-exit=1 \
 "$python_bin" -u -m workflows.sbi.e2e_stabilization worker --output "$run_root"
srun --jobid="$job_id" -N1 -n1 -c8 --gpus=1 \
 "$python_bin" -u -m workflows.sbi.e2e_stabilization collect --output "$run_root"
