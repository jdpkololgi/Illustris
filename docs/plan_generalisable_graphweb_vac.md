...
- [ ] Build and validate the ph001 random-response overlay on a compute node, then
  ...
  supervisor is active on CPU job `57872480`: the complete runtime and nine
  authority/builder tests pass and the 18-realisation scan has started. Its first
...
- [x] Stage `workflows/sbi/run_p12a_blind_context_tmux.sh` (commit `7f7695c`). Tmux
  session `blind_context` waits without an allocation for the response manifest, then
