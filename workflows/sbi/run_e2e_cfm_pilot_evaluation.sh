#!/bin/bash
set -euo pipefail
eval_root=/pscratch/sd/d/dkololgi/abacus/e2e_field_v3/cfm_pilot_eval_20260924_v1
cd "$eval_root/source"
if [[ ${1:-} != compute ]]; then
    set +e
    salloc --job-name=cfm_eval --account=desi_g --constraint=gpu --nodes=1 \
        --ntasks=4 --cpus-per-task=16 --gpus=4 --qos=interactive \
        --time=04:00:00 --licenses=scratch --immediate=600 \
        bash workflows/sbi/run_e2e_cfm_pilot_evaluation.sh compute > "$eval_root/allocation.log" 2>&1
    result=$?
...
if [[ $result == 0 ]]; then
    python -m workflows.sbi.e2e_cfm_pilot_eval_report --root "$eval_root/results" || result=1
fi
exit "$result"
