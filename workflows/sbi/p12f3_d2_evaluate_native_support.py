#!/usr/bin/env python3
"""D2 evaluation with the authorized native-galaxy guard; all field code frozen."""
...
# Import all original D2 science modules from the unchanged 467f442 worktree.
# The separately hashed entrypoint/helper implement only the authorized amendment.
sys.path.insert(0, "/global/u2/d/dkololgi/TNG/Illustris_d2_467f442")
from p12f3_d2_native_support import NativeSupport, require_native_record
...
def derived_physics_conditionals(
    ...
    for metadata, record in records:
        ...
        # Galaxy membership is verified at its native angular/radial position.
        # The exact voxel mask and all field operations remain unchanged.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--support-smoke", action="store_true")
...
def main() -> None:
    ...
    native = NativeSupport(args.support_amendment, evaluator=Path(__file__))
    if native.marker["files"]["base_contract"]["sha256"] != sha256(contract_path):
        raise RuntimeError("native amendment belongs to a different base contract")
    ...
    if args.support_smoke:
        audit = json.loads(Path(native.marker["files"]["geometry_audit"]["path"]).read_text())
        bad_cores = {int(row["core"]) for row in audit["rows"]}
        offsets = native.arrays["core_active_offsets"]
        core = max(bad_cores, key=lambda c: int(offsets[c+1]-offsets[c]))
        entry = next(row for row in archive["entries"] if int(row["core_id"]) == core)
        if sha256(Path(entry["path"])) != entry["sha256"]:
            raise RuntimeError("native support smoke archive changed")
        record = load_core_record(entry, 64)
        with np.load(entry["path"], allow_pickle=False) as values:
            record.update({name: np.asarray(values[name]) for name in PROXY_CONDITIONS})
        native.verify_record(record, core, archive["method"])
        from workflows.sbi.p12f3_d2_evaluate import derived_physics_conditionals as original_derived
        try:
            original_derived([(metadata[core], record)], device=args.device, seed=common_seed+300)
        except RuntimeError as error:
            if str(error) != "D2 derived calibration includes an M=0 galaxy":
                raise
        else:
            raise RuntimeError("smoke must reproduce the original nearest-voxel failure")
        derived_smoke = derived_physics_conditionals(
            [(metadata[core], record)], device=args.device, seed=common_seed+300)
        if derived_smoke["galaxies"] != len(record["galaxy_frac_index_local"]):
            raise RuntimeError("native smoke dropped galaxies")
        report = dict(schema_version="d2-native-support-smoke-v1", pass_=True,
            core=core, galaxies=derived_smoke["galaxies"], original_guard_rejected=True,
            amended_guard_passed=True, native_support=native.summary(),
            contract_sha256=sha256(contract_path), ph001_opened=False)
        report["pass"] = report.pop("pass_")
        output = args.output_dir / "NATIVE_SUPPORT_SMOKE_PASS.json"
        if output.exists():
            raise FileExistsError("native support smoke receipt already exists")
        atomic_json(output, report)
        print(json.dumps(report, indent=2))
        return
    output_dir = args.output_dir or args.archive.parent / "reports_native_support_v1"
    output_dir.mkdir(parents=True, exist_ok=True)
    evidence_path = output_dir / "D2_PH006_EVALUATION.json"
    evaluation_frozen = {
        "native_support_amendment": str(args.support_amendment.resolve()),
        "native_support_amendment_sha256": native.sha256,
    ...
    for entry in archive["entries"]:
        ...
        native.verify_record(record, int(entry["core_id"]), archive["method"])
    ...
    if derived["galaxies"] != 133698:
        raise RuntimeError("native support amendment must retain every audited galaxy")
    common["native_galaxy_support"] = native.summary(require_complete=True)
    ...
    for key in config["evaluation"]["matched_reference_methods"]:
        ...
        for entry in reference_archive["entries"]:
            ...
            native.verify_record(reference_record, core_id, key)
    ...
    evidence = {
        "native_support_amendment": str(args.support_amendment.resolve()),
        "native_support_amendment_sha256": native.sha256,
        "native_galaxy_support": native.summary(require_complete=True),
