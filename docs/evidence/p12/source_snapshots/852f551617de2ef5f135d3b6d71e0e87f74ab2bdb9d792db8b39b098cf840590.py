...
def replay(output):
    ...
    result=dict(schema='p12a-frozen-mock-replay-v1',phase='ph002',source=source,job=os.environ['SLURM_JOB_ID'],cudnn_allow_tf32=True,matmul_allow_tf32=False,
    ...
    for mode,contract,checkpoint in [('oof',Path(sr['contract_root']),Path(sr['checkpoint'])),('fullfit',BASE/'training_contract',BASE/'arm_a_training/arm_a_r0_v1/unet/seed_42/best_checkpoint.pt')]:
        ...
        for meta in chosen:
            ...
            if mode=='oof':
                ...
                diagnostics['patch_shape'] = list(patch.values.shape)
                diagnostics['available_halo_low'] = patch.available_halo_low.tolist()
                diagnostics['available_halo_high'] = patch.available_halo_high.tolist()
                if core in (370,13207):
                    reference64=adapter.extract(core,64,u.CHANNELS,alignment_voxels=8)
                    diagnostics['diagnostic_halo48_vs64']=compare(ref,predict(model,reference64,ck))
