#!/usr/bin/env python3
"""Report atomic-marker status for the ph001--ph005 P1--P4 campaign."""
...
PHASES = tuple(f"ph{i:03d}" for i in range(1, 6))
...
def record(root: Path, phase: str) -> dict:
    ...
    checks = {
        ...
        "phase_complete": p / ("BLIND_INPUT_COMPLETE.json" if blind else "PHASE_COMPLETE.json"),
    }
    if blind:
    ...
    return {"phase": phase, "blind": blind, "status": status,
...
def main() -> None:
    ...
    payload = {"schema_version": "p10-multiphase-status-v1",
