...
- [x] Build and validate the ph001 random-response overlay on a compute node. The first allocation
  attempts on 2026-09-02 were cancelled after both interactive partitions reported
  `PartitionDown`; do not move this workload onto a login node. The resumed tmux
  supervisor completed on CPU job `57872480`: the complete runtime and nine
  authority/builder tests passed, followed by the registered 18-realisation scan and
  two-cap overlay. Manifest SHA-256 is `708491fa...1c3570`; grid, support, finiteness,
  Poisson and shell-total QA gates all pass with `ph001_opened=false`. Its first
  bounded preflight on job `57872429` correctly stopped before scanning because the
  minimal CFS recovery environment lacks `healpy`.
- [x] Fix and compute-test the P12-A blind context path before production use. Commit
  `3039fbb` removes a checkpoint-record/base-width shadowing bug, binds the adapter to
  exact ph001 assignment/points/P3 hashes, enforces the train-only selection fit, and
  rejects cross-phase or truth-bearing sources. The synthetic context and core-safe
  shard tests pass; no ph001 prediction was generated by the test.
- [x] Harden the context exporter against the schemas actually frozen on disk. Commit
  `e0af07c` supports both normalized visible-phase and canonical P3b-R response
  manifests; commit `ce2caf2` binds the exact parent-aligned observed redshift array
  and sealed ph001 loader contract because geometry-only `points.npy` has four
  columns. The focused CFS-runtime tests pass.
- [x] Stage `workflows/sbi/run_p12a_blind_context_tmux.sh` (commit `7f7695c`, corrected
  by `ce2caf2`). Fresh tmux session `blind_context_v2` waits without an allocation, then
